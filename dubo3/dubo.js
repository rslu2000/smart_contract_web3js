...
let contractInstance = duboContract.at('0x2523127504a12dedef3fd968b89a8d8c53cb5cd9');
...
Rx.Observable.fromEvent($('#finishBtn'), 'click')
  .mergeMap(() => {
    ...
    return send('finish', address);
  }).retry().subscribe()

  Rx.Observable.fromEvent($('#deWaterBtn'), 'click')
  .mergeMap(() => {
    let address = document.getElementById('deWaterValue').value;
    return send('deWater', address);
  }).retry().subscribe()
